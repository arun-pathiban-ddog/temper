...
The session owner supervises reader, writer and dispatch together. Input failure interrupts dispatch even when output is blocked. Only clean input EOF may drain queued requests; transport errors abort the remaining tasks and propagate. Once the loop finishes, MCP executable entry points bound runtime shutdown to one second because native pipe calls cannot be canceled by Tokio. Real-process broken-output and pipe-saturation regressions cover this distinction alongside the in-memory tests.
